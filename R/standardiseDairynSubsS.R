...
standardiseDairynSubsS <- function(df) {
  df  %>%
    ...
    mutate(Ingredients_standardised = case_when(
      ...
      str_detect(Ingredients, 'lightroom 10 %|light flow 10 %') | str_detect(Ingredients, 'sour') & str_detect(Ingredients, 'cream') & str_detect(Ingredients, 'light|10%|10 %') ~ 'sour cream 10 %',
      ...
      str_detect(Ingredients, 'lightroom 18 %|light stream|light flow 18%|lightroom 18%|light flow 18 %|light r\u00f8mme') |
        (str_detect(Ingredients, 'sour cream|soured cream') & !str_detect(Ingredients, '%')) | str_detect(Ingredients, 'sour cream|soured cream') & str_detect(Ingredients, '18%|18 %|17%|17 %') |
        (str_detect(Ingredients, 'cream') & str_detect(Ingredients, 'light') & !str_detect(Ingredients, 'alternativ|sour')) |
        str_detect(Ingredients, 'sour') & str_detect(Ingredients, 'cream') & !str_detect(Ingredients, 'sandwich|nut') ~ 'sour cream 18 %', #Standard
