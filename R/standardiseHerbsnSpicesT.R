...
standardiseHerbsnSpicesT <- function(df) {
  df  %>%
    ...
    mutate(Ingredients_standardised = case_when(
      str_detect(Ingredients, 'taco') & str_detect(Ingredients, 'spice|season') ~ 'spice mix taco',
      str_detect(Ingredients, 'tandoori') & !str_detect(Ingredients, 'paste') ~ 'spice mix tandoori',
      str_detect(Ingredients, 'tarragon') & (str_detect(Ingredients, 'fresh|chop|crush|neve|twig|leaf') | str_detect(unit, 'twig|bunch|leaf|neve|dl')) ~ 'tarragon fresh herbs',
