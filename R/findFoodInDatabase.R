    ...
    if(nrow(tmp_db) == 0) {
      ...
      ID <- tmp_db %>%
        pivot_longer(.,
                     ...
                     names_to = "which_word",
                     values_to = "word") %>%
        mutate(Ingredients = singleIngredient) %>%
        dplyr::mutate(.by = database_ID,
                      new_ID = case_when(
                        all(str_detect(Ingredients, paste0("\\\\b", word))) ~ paste0(database_ID, "_bothRefWords"),
                        which_word == "database_referenceword1" &
                          str_detect(Ingredients, paste0("\\\\b", word)) ~ paste0(database_ID, "_onlyFirstWord"),
                        TRUE ~ "No hit"),
                      database_reference = case_when(
                        str_detect(new_ID, "Word") ~ paste0(word, collapse = " ")
                      )

                      ) %>%
        # Only keep new id
        dplyr::select(Ingredients, new_ID, database_reference) %>% distinct() %>%
        rename(database_ID = new_ID)
#
#       for (reference in 1:length(tmp_db)) {
#
#         #Only look for the whole word found in the reference
#         if(all(str_detect(singleIngredient, regex(paste0('\\b', tmp_db$first_word[reference], '\\b'))),
#                str_detect(singleIngredient, regex(paste0('\\b', tmp_db$second_word[reference], '\\b'))))
#         ) {
#           print("in loop 1")
#           ID <- tibble(
#             database_ID = tmp_db$database_ID[reference],
#             database_reference = paste0(tmp_db$first_word[reference], " ", tmp_db$second_word[reference])
#           )
#           break
#
#         } else if(all(str_detect(singleIngredient, regex(paste0('\\b', tmp_db$first_word[reference], '\\b'))),
#                       str_detect(singleIngredient, regex(paste0('\\b', tmp_db$second_word[reference], '\\w+'))))
#         ) {
#           print("in loop 2")
#           ID <- tibble(
#             database_ID = tmp_db$database_ID[reference],
...
#           break
#
#         } else if(str_detect(singleIngredient, regex(paste0('\\b', tmp_db$first_word[reference]))) ) {
#           print("in loop 3")
#           ID <- tibble(
#             database_ID = tmp_db$database_ID[reference],
#             database_reference = paste0(tmp_db$first_word[reference], " ", tmp_db$second_word[reference])
    ...
    if(database == 'volume_weight'){
      ...
      temp <- results %>%
        ...
        dplyr::mutate(to_fix = case_when(
          Ingredients %in% databaseHitFixes$volumeWeight$Ingredients ~ "ToFix",
          TRUE ~ "NoFix"
        )) %>%
        split(.$to_fix)
      ...
      if(all(is.data.frame(temp$ToFix) & nrow(temp$ToFix > 0))) {
        ...
        temp$ToFix <- temp$ToFix %>%
          left_join(sustainableNutRients:::databaseHitFixes$volumeWeight) %>%
          dplyr::mutate(
            ID = map2(database_referenceword1, database_referenceword2, function(word1, word2) {

              new_ID <- fixFoodMappingError(database = reference, first_w = word1, second_w = word2)
      ...
      temp <- bind_rows(temp) %>%
        ...
        dplyr::mutate(
          database_ID = case_when(
            !!!rlang::parse_exprs(sustainableNutRients:::not_in_database %>% dplyr::filter(database == "volumeWeight") %>% pull(expression)),
            TRUE ~ database_ID
        ))
      ...
      temp <- bind_row(temp) %>%
        ...
        # Check for food items not i database
        dplyr::mutate(
          database_ID = rlang::parse_exprs(sustainableNutRients:::not_in_database %>% dplyr::filter(database == "nutrients") %>% pull(expression)),
          TRUE ~ database_ID
        )
      ...
      temp <- bind_row(temp) %>%
        ...
        # Check for food items not i database
        dplyr::mutate(
          database_ID = rlang::parse_exprs(sustainableNutRients:::not_in_database %>% dplyr::filter(database == "sustainability") %>% pull(expression)),
          TRUE ~ database_ID
        )
...
# function(df, database, additional_entries = NULL, fix_errors = TRUE, include_water = FALSE){
#
# # Source fixes for known errors
# source("./fixFoodItemsLists.R")
#
