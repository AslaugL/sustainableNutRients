...
#' @param df
#' - For volume_weight, a dataframe three columns "Ingredients", "unit" and "grams_per_unit",
#' - For "nutrients", the output from calculateNutrientContentOfFoodlist with option "per 100 g" selected or a three column dataframe with
...
#' - For "sustainability" the output from calculateEnvironmentalImpactOfFoodlist with option "per 100 g"
...
#'
#' @export
createNewDatabaseEntry <- function(database, df) {

  #Function to format the database entries
  ...
  if(database == 'volume_weight') {
    ...
    if(isTRUE(all.equal(names(df), c('Ingredients', 'unit', 'grams_per_unit')))) {
      ...
      stop("To create database entries for the volume_weight database, df must include the columns Ingredients, unit and grams_per_unit.")
    }
